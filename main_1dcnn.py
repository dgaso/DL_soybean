...
import glob
from src.lstm import TemporalConvolutionalNetwork, TimeSeriesCNN
...
parser = argparse.ArgumentParser(description="Training of LSTM")

data_dir='byYear_v2'  #    byfieldID,  byYear 
y = 'out2020'  #    byfieldID,  out2022  
inputs = 'RS'
fold = '_fold4'
results = 'years' #  byfieldID  years  
predname = f'{y}_{inputs}_1DCNN'

#########################
#### data parameters ####
#########################
parser.add_argument("--dump_path", type=str, default=f"./exp_{results}/{y}_{inputs}_1DCNN",
                    help="experiment dump path for checkpoints and log")


parser.add_argument('--data_train', type=str, default=f'./data/{data_dir}/{y}_{inputs}_data_train.npy',
                        help="Path containing the raster image")
parser.add_argument('--target_train',type=str, default=f'./data/{data_dir}/{y}_{inputs}_target_train.npy', 
                    help="Path containing the refrence label image for training")
parser.add_argument('--data_val', type=str, default=f'./data/{data_dir}/{y}_{inputs}_data_val.npy',
                        help="Path containing the raster image")
parser.add_argument('--target_val',type=str, default=f'./data/{data_dir}/{y}_{inputs}_target_val.npy', 
                    help="Path containing the refrence label image for training")
...
parser.add_argument("--batch_size", default=256, type=int, ## batch size:16,32,64,128,256,512,1024 !!16 in120m and 512 in 20m
                    help="batch size per gpu, i.e. how many unique instances per gpu")
parser.add_argument("--base_lr", default=0.00001, type=float, help="base learning rate") ####################### IMPORTANTE
...
def main(hs,bl,nl,op,so):
    ...
    # build model
    model = TimeSeriesCNN(input_size=data_train.shape[1], 
                      hidden_size = args.hidden_size,                       
                      output_size = args.output_size,
                      kernel_size=data_train.shape[2],
                      n_layers = args.num_layers,
                      fc_dropout = 0.0,
                      sigm_out = args.sigm_out,
                      time=data_train.shape[-1])
